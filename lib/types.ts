export type Canton = {
  id: string;
  name: string;
  slug: string;
  abbreviation: string;
  description: string;
};

export type City = {
  id: string;
  name: string;
  slug: string;
  cantonSlug: string;
  postalCode: string;
  description: string;
};

export type CivilRegistryOffice = {
  id: string;
  name: string;
  slug: string;
  cantonSlug: string;
  citySlug: string;
  addressLine1: string;
  postalCode: string;
  city: string;
  phone: string;
  email: string;
  websiteUrl: string;
  appointmentUrl?: string;
  openingHours?: string;
  languages: string[];
  responsibleMunicipalities: string[];
  description: string;
  documentsInfo: string;
  ceremonyInfo: string;
  sourceUrl: string;
  lastVerifiedAt: string;
};

export type GuideArticle = {
  title: string;
  slug: string;
  excerpt: string;
  metaTitle?: string;
  metaDescription?: string;
  content: string[];
  faq: { question: string; answer: string }[];
  sourceName?: string;
  sourceUrl?: string;
  sourceDescription?: string;
  officialLinks?: { title: string; description: string; url: string; meta?: string }[];
};

export type Vendor = {
  companyName: string;
  slug: string;
  category: string;
  region: string;
  cantonSlug: string;
  shortDescription: string;
  websiteUrl: string;
  priceRange: string;
  languages: string[];
  styleTags: string[];
  featured: boolean;
};

export type ImageStatus = "approved" | "needs_review" | "fallback_crest" | "placeholder";

export type LicensedImageFields = {
  imageUrl?: string;
  imageAlt?: string;
  imageSource?: string;
  imageLicense?: string;
  imageAttribution?: string;
  imageStatus?: ImageStatus;
  /** Explicit rights approval for public display without a visible credit line. */
  publicDisplayWithoutCreditApproved?: boolean;
  galleryImages?: Array<{
    url: string;
    alt?: string;
    publicDisplayWithoutCreditApproved: boolean;
  }>;
  imageCandidateUrl?: string;
  imageCandidateNote?: string;
  imageCandidateStatus?: "manual_permission_required";
};

export type RegistryCanton = {
  code: string;
  name: string;
  slug: string;
  officeCount: number;
  municipalityCount: number;
  map: number[];
  coatOfArmsUrl?: string;
  mediaAlt?: string;
};

export type SwissRegistryOffice = LicensedImageFields & {
  canonicalId?: string;
  id: string;
  name: string;
  slug: string;
  canton: string;
  cantonName: string;
  postalCode: string;
  city: string;
  addressLine1: string;
  postBox: string;
  phone: string;
  fax: string;
  email: string;
  officialUrl: string;
  website_url?: string;
  marriage_info_url?: string;
  appointment_url?: string;
  documents_url?: string;
  faq_url?: string;
  openingHours?: string;
  ceremonyLocations?: string[];
  ceremonyTimes?: string;
  wheelchairAccessible?: string;
  parking?: string;
  onlineCalendarAvailable?: string;
  onlineCalendarUrl?: string;
  ceremonyMonday?: boolean | null;
  ceremonyTuesday?: boolean | null;
  ceremonyWednesday?: boolean | null;
  ceremonyThursday?: boolean | null;
  ceremonyFriday?: boolean | null;
  ceremonySaturday?: boolean | null;
  ceremonySaturdayStatus?: string;
  ceremonySaturdaySourceUrl?: string;
  ceremonySaturdayResearchNotes?: string;
  ceremonySaturdayResearchCheckedAt?: string;
  ceremonySunday?: boolean | null;
  eveningCeremonyAvailable?: boolean | null;
  ceremonyByAppointmentOnly?: boolean | null;
  outdoorCeremonyAvailable?: boolean | null;
  outdoorCeremonyStatus?: string;
  outdoorCeremonySourceUrl?: string;
  maxCeremonyGuests?: number | null;
  multipleCeremonyVenuesAvailable?: boolean | null;
  multipleCeremonyVenuesStatus?: string;
  multipleCeremonyVenuesSourceUrl?: string;
  ceremonyVenueCount?: number | null;
  ceremonyVenueCountLabel?: string;
  ceremonyVenueNotes?: string;
  ceremonyVenueSeasonalAvailability?: string;
  ceremoniesOutsideOfficeHours?: boolean | null;
  onlineAppointmentBookingAvailable?: boolean | null;
  onlineAppointmentBookingStatus?: string;
  onlineAppointmentBookingSourceUrl?: string;
  appointmentBookingUrl?: string;
  wheelchairAccessibleBoolean?: boolean | null;
  wheelchairAccessibleSourceUrl?: string;
  parkingAvailableBoolean?: boolean | null;
  ceremonyRemarks?: string;
  phase1ResearchNotes?: string;
  phase1CheckedAt?: string;
  coatOfArmsUrl?: string;
  mediaAlt?: string;
  mediaLicenseNote?: string;
  responsibleMunicipalities: string[];
  map: number[];
};

export type CeremonyVenue = LicensedImageFields & {
  coatOfArmsUrl?: string;
  mediaAlt?: string;
  ceremonyDaysNote?: string;
  ceremonyTimes?: string;
  capacityNote?: string;
  parkingDescription?: string;
  indoor?: boolean | null;
  reservationRequired?: boolean | null;
  canonicalId?: string;
  standesamt_id: string;
  standesamt_name: string;
  traulokal_name: string;
  adresse: string;
  ort: string;
  kanton: string;
  beschreibung: string;
  ceremonyMonday: boolean | null;
  ceremonyTuesday: boolean | null;
  ceremonyWednesday: boolean | null;
  ceremonyThursday: boolean | null;
  ceremonyFriday: boolean | null;
  ceremonySaturday: boolean | null;
  ceremonySunday: boolean | null;
  eveningCeremonyAvailable: boolean | null;
  maxCeremonyGuests: number | null;
  wheelchairAccessible: boolean | null;
  parkingAvailable: boolean | null;
  outdoorCeremonyAvailable: boolean | null;
  seasonalAvailability: string;
  venueUrl: string;
  officialConfirmed?: boolean | null;
  sourceUrl?: string;
  sourceType?: string;
  beautyStatus?: string;
  highlightLevel?: number | null;
  tags?: string[];
  websitePriority?: string;
  remarks?: string;
};
