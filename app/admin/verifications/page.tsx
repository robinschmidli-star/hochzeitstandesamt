...
import { publicOfficesForVerification } from "@/lib/verification";

export const dynamic="force-dynamic";
type Proposal={id:string;entity_name:string;field_name:string;old_value:unknown;proposed_value:unknown;source_name:string;created_at:Date};
export default async function VerificationsAdminPage(){
  const [offices,proposals,requests]=await Promise.all([
    ...
    prisma.$queryRaw<Proposal[]>`SELECT p.id::text,COALESCE(v.name,o.name) entity_name,p.field_name,p.old_value,p.proposed_value,r.source_name,p.created_at FROM verification_change_proposals p JOIN verification_requests r ON r.id=p.verification_request_id LEFT JOIN wedding_venues v ON p.entity_type='wedding_venue' AND v.id=p.entity_id LEFT JOIN civil_registry_offices o ON p.entity_type='civil_registry_office' AND o.id=p.entity_id WHERE p.status='pending' ORDER BY p.created_at`,
    prisma.$queryRaw<Array<{id:string;office_name:string;venue_name:string|null;status:string;language_code:string;expires_at:Date}>>`SELECT r.id::text,o.name office_name,v.name venue_name,r.status,r.language_code,r.expires_at FROM verification_requests r JOIN civil_registry_offices o ON o.id=r.office_id LEFT JOIN wedding_venues v ON v.id=r.venue_id ORDER BY r.created_at DESC LIMIT 50`
  ]);
