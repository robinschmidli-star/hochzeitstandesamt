...
import { VerificationBatch } from "@/components/VerificationBatch";
...
export default async function VerificationsAdminPage(){
  const [offices,proposals,requests]=await Promise.all([
    ...
    prisma.$queryRaw<Array<{id:string;office_id:string;source_name:string;venue_id:string|null;status:string;language_code:string;expires_at:Date}>>`SELECT id::text,office_id::text,source_name,venue_id::text,status,language_code,expires_at FROM verification_requests ORDER BY created_at DESC LIMIT 50`
  ]);
  ...
  return <main className="mx-auto grid max-w-6xl gap-7 px-4 py-10 sm:px-6"><div><p className="text-sm font-semibold uppercase tracking-widest text-champagne">Admin</p><h1 className="mt-2 text-4xl font-semibold">Datenverifikation</h1></div><VerificationBatch/><VerificationAdminCreate offices={offices}/>
    ...
    <section><h2 className="mb-3 text-2xl font-semibold">Letzte Links</h2><div className="overflow-x-auto rounded-xl border border-linen bg-white"><table className="min-w-full text-left text-sm"><thead><tr><th className="p-3">Amt / Trauort</th><th>Sprache</th><th>Status</th><th>Ablauf</th><th>Aktion</th></tr></thead><tbody>{requests.map(r=><tr key={r.id} className="border-t border-linen"><td className="p-3">{r.source_name}</td><td>{r.language_code.toUpperCase()}</td><td>{r.status}</td><td>{r.expires_at.toLocaleDateString("de-CH")}</td><td>{r.venue_id===null&&r.status==="active"?<form action="/admin/api/verification-requests/replace" method="post"><input type="hidden" name="officeId" value={r.office_id}/><input type="hidden" name="language" value={r.language_code}/><button className="rounded-lg bg-sage px-3 py-1 text-xs font-semibold text-white">Ersetzen</button></form>:null}</td></tr>)}</tbody></table></div></section>
  </main>;
}
