...
export const metadata = { title: "Trauort-Datenprüfung", robots: { index: false, follow: false } };

export default async function VenueVerificationPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const request = await requestByToken(token);
  if (!request?.venueId) notFound();
  ...
  return <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
    ...
    <h1 className="mt-2 text-3xl font-semibold text-ink sm:text-4xl">{labels["verify.venueTitle"].replace("{name}", request.snapshot.venues[0].name)}</h1>
